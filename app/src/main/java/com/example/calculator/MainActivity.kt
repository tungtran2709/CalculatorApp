...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun setOnClickButtion() = with(binding) {
            listOf(
    ...
    override fun onClick(v: View?): Unit = with(binding) {
        when (v) {
            btnZero -> addTextCalculator(btnZero.text.toString())
            btnOne -> addTextCalculator(btnOne.text.toString())
            btnTwo -> addTextCalculator(btnTwo.text.toString())
            btnThree -> addTextCalculator(btnThree.text.toString())
            btnFour -> addTextCalculator(btnFour.text.toString())
            btnFive -> addTextCalculator(btnFive.text.toString())
            btnSix -> addTextCalculator(btnSix.text.toString())
            btnSeven -> addTextCalculator(btnSeven.text.toString())
            btnEight -> addTextCalculator(btnEight.text.toString())
            btnNine -> addTextCalculator(btnNine.text.toString())
            btnDot -> addTextCalculator(btnDot.text.toString())
            btnPercent -> addTextCalculator(btnPercent.text.toString())
            btnDiv -> addTextCalculator(btnDiv.text.toString())
            btnMul -> addTextCalculator(btnMul.text.toString())
            btnSub -> addTextCalculator(btnSub.text.toString())
            btnAdd -> addTextCalculator(btnAdd.text.toString())
            btnClear -> clearScreen()
            btnDelete -> delete()
            btnEqual -> {
                val arrayPostfix = convertPostfix(txtCalculator.text.toString())
